...
class Enemy:
    ...
    def draw(self, surface, font_manager, asset_manager):
        # Draw Enemy Image (match rect size)
        img = asset_manager.get_image("enemy_1", self.rect.width, self.rect.height)
...
class Battle:
    ...
    def __init__(self, deck_manager, font_manager, stage, player, save_manager, sound_manager, asset_manager, on_battle_end, on_game_over):
        ...
        self.sound_manager.play_bgm("bgm_battle.wav")
    ...
    def handle_correct(self, card):
        ...
        # Visual Effect
        self.visual_effects.create_sparkle(self.main_area_center_x, 300)
    ...
    def handle_enemy_turn(self):
        ...
        # Explosion Effect
        rect = self.player_rect if hasattr(self, 'player_rect') else pygame.Rect(self.main_area_center_x, SCREEN_HEIGHT - 100, 10, 10)
        self.visual_effects.create_explosion(rect.centerx, rect.centery)
    ...
    def draw(self, screen):
        # Draw Background
        bg_img = self.asset_manager.get_image("bg_battle", SCREEN_WIDTH, SCREEN_HEIGHT)
        ...
        # Enemy
        self.enemy.draw(screen, self.font_manager, self.asset_manager)
        
        # Visual Effects
        self.visual_effects.draw(screen)
        ...
        if self.active_card_index < len(self.deck_manager.hand):
            ...
            pygame.draw.rect(s, (0, 0, 0, 80), s.get_rect(), border_radius=15)
            screen.blit(s, shadow_rect)
            
            # Card Body
            pygame.draw.rect(screen, (250, 250, 252), card_rect, border_radius=15)
            pygame.draw.rect(screen, (100, 100, 120), card_rect, width=2, border_radius=15)
            
            # Question Text
            q_font = self.font_manager.get_font(64, 'english')
            display_text = current_card.get_display_text() 
            q_surf = q_font.render(display_text, True, (40, 40, 50))
